...
struct imageRegion {
  ...
  int threadID;
  int totalThreads;
  // 2-D array holding color values
  struct rgb** colors;
};

void *printFullImage(struct rgb **image) {
  int i;
  int j;
  for (i = 0; i < width; ++i) {
    for (j = 0; j < height; ++j) {
...
void *processImage(void *arg) {
  struct imageRegion *image_region = (struct imageRegion *)arg;
  int region_width = image_region->endWidth;
  int start_width = image_region->startWidth;
  scene_t scene = image_region->scene;
  int threadID = image_region->threadID;
  struct rgb** colors = image_region->colors;
  
  //  int width = region_width - start_width;
    ...
    /* for every pixel */
    for( int px=start_width; px<region_width; ++px )
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
	    //	    colors[px][py] = malloc(sizeof(struct rgb));

	      colors[px][py].r = scaled_color[0];
	      colors[px][py].g = scaled_color[1];
	      colors[px][py].b = scaled_color[2];
	    
	    //	    printf("thread id: %d\n", threadID);
	    if (image_region->totalThreads < 2) {
	      printf( "%.0f %.0f %.0f\n",
...
main( int argc, char **argv )
{
    ...
    printf( "# Rendering scene with %d spheres and %d lights\n",
      ...
      // Allocate memory to hold pointers to float color values for
      // every pixel in that region
    struct rgb** temp = malloc((width * height * sizeof(struct rgb)));
    ...
    int k;
    for (k = 0; k < width; ++k) {
	temp[k] = (struct rgb *)malloc(height * sizeof(struct rgb));
	if (!temp[k])
	  exit(1);
    }
    ...
    int columnWidth = width / nthreads;

    pthread_t tids[nthreads];
    struct imageRegion *region[nthreads];
    int i;
    for (i = 0; i < nthreads; ++i) {
      // Make struct to hold all arguments
      region[i] = malloc(sizeof(struct imageRegion));
      region[i]->startWidth = i * columnWidth;
      region[i]->scene = scene;
      region[i]->threadID = i;
      region[i]->totalThreads = nthreads;

      if (i != nthreads - 1)
	region[i]->endWidth = region[i]->startWidth + columnWidth;
      else
	region[i]->endWidth = width;

      region[i]->colors = temp;

      // Make thread
      int ret = pthread_create(&tids[i], NULL, processImage, region[i]);
      if(ret) {
	...
	ret2 = pthread_join(tids[i], printFullImage(temp));
      else
	ret2 = pthread_join(tids[i], NULL);
      if (ret2) {
